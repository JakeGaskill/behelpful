	...
	for($list_day = 1; $list_day <= $days_in_month; $list_day++):
		$calendar.= '<td class="calendar-day"><div style="position:relative;height:100px;">';
			...
			if(isset($events[$event_day])) {
                    ...
                    {
                        $go=$_SESSION['GOVID'];
                        $query4 = queryMysql("SELECT * FROM CALENDER  WHERE YEAR='$year' AND MONTH='$month' AND DAY='$list_day' AND GOVERNMENTID='$go' ORDER BY TIMESTAMP");

                        while($row  = $query4->fetch_assoc())
                        {
                        $calendar.= '<div class="event" ><a href="view.php?viewid='.$row['POSID'].'">' . $row['POSID'].'</a> - '.$row['USER'] .'- '.$row['HOUR'].":".$row['MIN']." ".$row['AM_PM']. '</div>';
                        }
                    }
                    else
                    {
                        
                        $query4 = queryMysql("SELECT POSID, YEAR, MONTH, DAY, HOUR, MIN, AM_PM FROM CALENDER  WHERE YEAR='$year' AND MONTH='$month' AND DAY='$list_day' AND USER='$us' ORDER BY TIMESTAMP");

                        while($rowl  = $query4->fetch_assoc())
                        {
                        $calendar.= '<div class="event" ><a href="view.php?viewid='.$rowl['POSID'].'">' . $rowl['POSID'].'</a>-'.$rowl['HOUR'].":".$rowl['MIN']." ".$rowl['AM_PM']. '</div>';
                        }
                    }
...
function volunteer($connection)
{
  $out="<span id='feedback'></span>";
    ...
    while($row = $result->fetch_assoc())
    {
        ...
        $posid=$row['POSID'];
        $out = $out . '<div class="topper"><h4><a href="view.php?viewid='.$posid.'">' . $row['JOB_NAME'] . '</a><span class="rightr">' . $taken . '/' .$row['NEED'] . '</span></h4></div>'; 
        
        $r2 = queryMysql("SELECT USER FROM VOLUNTEER WHERE POSID='$posid'");
        while($row2=$r2->fetch_assoc())
        {
            $out = $out . "<div class='topper'  id='" . $row2['USER'] . "'>". $row2['USER']."&emsp;&emsp;&emsp;&emsp;&emsp;<button type='submit' value='" . $posid . "'  onclick=\"deletev(this,'" . $row2['USER'] . "')\">delete</button></div>";
        }
    }
        
    return $out;
}
function app_over($connection)
{
    
    $out="";
    $time = $_SERVER['REQUEST_TIME'];
     $out="<span id='feedback'></span>";
    $out=$out."<div id='apps' class='dropdown'><a href='#' onclick='appshow()'  ><h1>APPLICATIONS<i class='fa'></i> <b class='caret'></b></h1></a>
                    <div id=\"apps1\">";
    $govidd=$_SESSION['GOVID'];
    $query = "SELECT POSID, JOB_NAME, TIME_STAMP, NEED, AVAILABLE FROM FULLVIEW WHERE GOVERNMENTID='$govidd' ORDER BY TIME_STAMP ASC";
    if(!$result = $connection->query($query))
    ...
    while($row = $result->fetch_assoc())
    {
      $taken=$row['NEED'] - $row['AVAILABLE'];
        $posid=$row['POSID'];
        $out = $out . '<div class="topper"><h4><a href="view.php?viewid='.$posid.'">' . $row['JOB_NAME'] . '</a><span class="rightr">' . $taken . '/' .$row['NEED'] . '</span></h4></div>';
        $r2 = queryMysql("SELECT USER FROM INPROGRESS WHERE POSID='$posid'");
        while($row2=$r2->fetch_assoc())
        {
            $out = $out . "<div id='feedback_". $row2['USER'] ."'></div><div class='topper'  id='sec_". $posid . $row2['USER'] . "'>". $row2['USER']."&emsp;&emsp;&emsp;&emsp;&emsp;<button type='submit' value='" . $posid . "'  onclick=\"accept(this,'" . $row2['USER'] . "')\">Approve</button>&emsp;<button type='submit' value='" . $posid . "'  onclick=\"decline(this,'" . $row2['USER'] . "')\">Decline</button>&emsp;<button type='submit' value='" . $posid . "'  onclick=\"showtime(this,'" . $row2['USER'] . "')\">Interview</button><div class='settimerinter' id='set_" . $posid . "_" . $row2['USER'] ."'><br><br> <select id='month_".$posid."_".$row2['USER']."'  onchange=\"daysset('".$posid."','".$row2['USER']."')\"><option selected disabled>Select one</option><option>1 </option><option>2</option><option>3</option><option>4</option><option>5</option><option>6</option><option>7</option><option>8</option><option>9</option><option>10</option><option>11</option><option>12</option>  </select>&nbsp;/&nbsp;<select  id='day_".$posid."_".$row2['USER']."'><option selected disabled>Select one</option><option>1 </option><option>2</option><option>3</option><option>4</option><option>5</option><option>6</option><option>7</option><option>8</option><option>9</option><option>10</option><option>11</option><option>12</option><option>13 </option><option>14</option><option>15</option><option>16</option><option>17</option><option>18</option><option>19</option><option>20</option><option>21</option><option>22</option><option>23</option><option>24</option><option>25 </option><option>26</option><option>27</option><option>28</option><option>29</option><option>30</option><option>31</option></select>&nbsp;/&nbsp;<select id='year_".$posid."_".$row2['USER']."' onchange=\"daysset('".$posid."','".$row2['USER']."')\" ><option>" . date("Y",$time) ."</option>" ;
            for( $i=1; $i<5;$i++)
            {
                $iu=date("Y",$time) +$i;
                $out=$out . "<option>" . $iu."</option>";
            }
            
            $out=$out."</select>&emsp;&emsp;<select id='hour_".$posid."_".$row2['USER']."'><option selected disabled>Select one</option><option>1 </option><option>2</option><option>3</option><option>4</option><option>5</option><option>6</option><option>7</option><option>8</option><option>9</option><option>10</option><option>11</option><option>12</option></select>&nbsp;:&nbsp;<select id='min_".$posid."_".$row2['USER']."'><option selected disabled>Select one</option><option>00 </option><option>15</option><option>30</option><option>45</option> </select>&nbsp;<select id='pa_".$posid."_".$row2['USER']."'><option selected disabled>Select one</option><option>A.M.</option><option>P.M.</option> </select><br><br><div><button  type='submit' value='".$posid."' onclick=\"sub_inter(this,'".$row2['USER']."')\" >submit</button></div></div></div>";
        }
    }
    
    $out=$out."</div></div><div id='inter' class='dropdown'><a href='#' onclick='intershow()'  ><h1>INTERVIEW<i class='fa'></i> <b class='caret'></b></h1></a>
                    <div id=\"inter1\">";
    ...
    while($row = $result->fetch_assoc())
    {
      $taken=$row['NEED'] - $row['AVAILABLE'];
        $posid=$row['POSID'];
        $out = $out . '<div class="topper"><h4><a href="view.php?viewid='.$posid.'">' . $row['JOB_NAME'] . '</a><span class="rightr">' . $taken . '/' .$row['NEED'] . '</span></h4></div>';
        $r2 = queryMysql("SELECT USER FROM INTERVIEW WHERE POSID='$posid'");
        while($row2=$r2->fetch_assoc())
        {
            $is=$row2['USER'];
            $r1 = queryMysql("SELECT * FROM CALENDER WHERE POSID='$posid' AND USER='$is'");
            $ab=$r1->fetch_assoc();
            $out = $out . "<div id='feedback_". $row2['USER'] ."'></div><div class='topper'  id='sec_". $posid . $row2['USER'] . "'>". $row2['USER']."&emsp;&emsp;&emsp;&emsp;&emsp;<button type='submit' value='" . $posid . "'  onclick=\"accept(this,'" . $row2['USER'] . "')\">Approve</button>&emsp;<button type='submit' value='" . $posid . "'  onclick=\"decline(this,'" . $row2['USER'] . "')\">Decline</button>&emsp;<button type='submit' value='" . $posid . "'  onclick=\"showtime(this,'" . $row2['USER'] . "')\">Interview</button><br><br><div>".$ab['MONTH'] ." / ".$ab['DAY']. " / ".$ab['YEAR']." - ".$ab['HOUR'].":".$ab['MIN']." ".$ab['AM_PM']."</div><div class='settimerinter' id='set_" . $posid . "_" . $row2['USER'] ."'><br><br> <select id='month_".$posid."_".$row2['USER']."'  onchange=\"daysset('".$posid."','".$row2['USER']."')\"><option selected disabled>Select one</option><option>1 </option><option>2</option><option>3</option><option>4</option><option>5</option><option>6</option><option>7</option><option>8</option><option>9</option><option>10</option><option>11</option><option>12</option>  </select>&nbsp;/&nbsp;<select  id='day_".$posid."_".$row2['USER']."'><option selected disabled>Select one</option><option>1 </option><option>2</option><option>3</option><option>4</option><option>5</option><option>6</option><option>7</option><option>8</option><option>9</option><option>10</option><option>11</option><option>12</option><option>13 </option><option>14</option><option>15</option><option>16</option><option>17</option><option>18</option><option>19</option><option>20</option><option>21</option><option>22</option><option>23</option><option>24</option><option>25 </option><option>26</option><option>27</option><option>28</option><option>29</option><option>30</option><option>31</option></select>&nbsp;/&nbsp;<select id='year_".$posid."_".$row2['USER']."' onchange=\"daysset('".$posid."','".$row2['USER']."')\" ><option>" . date("Y",$time) ."</option>" ;
            for( $i=1; $i<5;$i++)
            {
                $iu=date("Y",$time) +$i;
                $out=$out . "<option>" . $iu."</option>";
...
function fullview($connection,$viewid)
{
    ...
    $out=$out."'class='img-circle img-responsive'> </div><div class=' col-md-9 col-lg-9 '><table class='table table-user-information'><tbody><tr><td>Job Position:</td><td>" . $row['JOB_NAME'] . "</td></tr><tr><td>Job Description</td><td>" . $row['FULL_DESCRIPTION'] . "</td></tr><tr><td>Position opened:</td><td>" . $date ."</td></tr><tr><td>Address</td><td>" . $row2['ADRESS']  . "</td></tr><tr><td>City</td><td>" . $row2['CITY'] . "</td></tr><tr><td>State</td><td>" . $row2['STATE'] . "</td></tr><tr><td>Zip code</td><td>" . $row2['ZIP_CODE'] . "</td></tr><tr><td>Positions Available</td><td>" . $row['AVAILABLE'] . "</td></tr><tr><td>Website</td><td>" . $row2['WEBSITE'] . "</td></tr><tr><td>Organization</td><td>" . $row['ORGANIZATION'] . "</td></tr><tr><td>Organization Bio</td><td>" . $row2['DESCRIPTION'] . "</td></tr><tr><td>Phone</td><td>" . $row2['PHONEH'] . "</td></tr><tr><tr><td>Fax</td><td>" . $row2['PHONEF'] . "</td></tr></tbody></table>";
    if($_SESSION['TYPE']==="WORKER")
    {
       $out=$out.""; 
    }
    ...
    {
        $out=$out. "<button type='submit' id='save_button' value='" . $row['POSID'] . "' onclick=\"Save(this,'" . $us . "')\" class='btn btn-primary";
...
function history_i($connection)
{
    
    $us=$_SESSION['USERNAME'];
    $query = "SELECT USER, POSID, TIME_STAMP FROM HISTORY WHERE USER='$us' ORDER BY TIME_STAMP DESC";
    if(!$result = $connection->query($query))
    {die('There was an error running the query [' . $_connection->error . ']');}
    $output = '';
    $time = $_SERVER['REQUEST_TIME'];
    while($row = $result->fetch_assoc())
    {
        $posid=$row['POSID'];
            ...
            $result4 = queryMysql("SELECT ORGANIZATION, POSID, JOB_NAME, JOB_DESCRIPTION, TIME_STAMP, CITY, STATE FROM SEARCHVIEW WHERE POSID='$posid'");
            $rowl = $result4->fetch_assoc();
            $output = $output . '<div class="topper"><a href="view.php?viewid=' . $row['POSID'] . '">' . $rowl['JOB_NAME'] . ' </a>- ' . $rowl['CITY'].'-'.$rowl['STATE'] . '<div>' . $rowl['JOB_DESCRIPTION'] . '</div> Added to list ' . ago($row['TIME_STAMP']) . '</div>';
...
<form method="post" action="approved.php">
  <input type="radio" name="job" value="accept_'.  $posid .'" checked> Accept
  <input type="radio" name="job" value="reject_'.  $posid .'"> Reject<br>
  <input type="submit">
</form> </div> </div>';
        }
        else
    ...
    if(!isset($job) ||!isset($location))
       {
        ...
        {
        $html="";
        $sql="SELECT * FROM  `FULLVIEW` WHERE STATE LIKE '%$location%' OR JOB_NAME LIKE '%$job%'";
       // echo($sql);
        //return;
        $result=queryMysql($sql);
        while($row=$result->fetch_assoc())
        {
            ...
            $html.=$row['JOB_NAME'] .' '.$row['FULL_DESCRIPTION'].' '.$row['CITY']. ' <a href="/view.php?viewid='.$row['POSID'].'">Click Here For Job</a> </br>';
